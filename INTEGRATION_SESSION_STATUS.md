...
## 1.5 CLI-only agent logic ported into the live backend (2026-09-05)

Two behaviors that existed only in the standalone CLI (`agents/agent_dispatcher.py`, `agents/agent_loop.py` — never invoked by the API or the web app) are now wired into the live backend agent path (`backend/app/agents/dispatcher.py`) and exposed over the API:

- **Seasonal Agent** (`seasonal_agent()`): the CLI shifted its flagging threshold by ±0.05 based on `datetime.now().month` (real wall-clock time), but was never called by the backend — the live system ran on a flat, un-adjusted threshold year-round. Ported into `backend/app/services/metrics.py::AnalysisContext`, with one deliberate adaptation: it now keys off the **analysis month being investigated** (`self.month.month`, e.g. 2024-12) rather than the operator's real-world clock, since the two are unrelated in a simulated system. Toggle: `ISTIKSHAF_SEASONAL_AGENT=on|off` (default `on`). Verified live: analysis month 2024-12 → "Winter" → threshold 0.50 → 0.55, flagged-consumer count drops from 545 to 481 (confirmed via direct scorer query). Exposed on `GET /api/overview` as `season`, `baseRiskThreshold`, `activeRiskThreshold`.
- **Recidivist probability boost**: the CLI's `dispatch_agent()` elevated a repeat offender's *effective* probability by +0.05 (capped at 1.0) before the soft-warning/routing decision; the backend only ever appended a "[REPEAT OFFENDER]" text annotation without it affecting anything. Ported into `run_dispatch()` as `effective_probability`, now driving the soft-warning band check and the field-alert wording. The raw model probability (`score.probability`) is left untouched everywhere else — only the dispatch *decision* uses the boosted number.
- Both are now surfaced on `GET /api/investigations/{id}/explanation` (`RiskExplanation`), which previously discarded `routing_decision`, `cancelled`, `is_recidivist`, `is_duplicate` even though `DispatchResult` computed them — an API consumer had no way to tell a cancelled/deduped/recidivist case apart from a normal one. Verified via a **direct call to `investigation_service.get_explanation()`**: `routing_decision="Routed to Analyst + Field"`, `is_recidivist=True`, `effective_probability=0.7827` (base 0.7327 + 0.05 boost) for consumer C-000853.

**Not yet verified**: a follow-up check hitting the same endpoint over HTTP via `TestClient` returned `null` for all four new `RiskExplanation` fields on a *different* consumer (the top of `/api/investigations`), while the direct service call above returned correct values. This wasn't root-caused before this session ended — needs re-checking (rule out route caching, a stale `response_model`, or a genuinely different code path for that consumer) before trusting these fields over HTTP for every case.

Still not ported / still open from the original CLI: the CLI's `check_confound()` / `case_dedup_guard()` write to a flat `audit_log.jsonl` file and only *print* + skip dispatch — the backend's confound cancellation still doesn't remove cases from the main investigation queue (see the Confounder Suppression row below), and the dedup guard bug below is a separate, pre-existing issue in the backend's own port, not something newly introduced by this session's changes.
...
- **Re-confirmed live (2026-09-05)**: ran the identical full-grid analysis twice back-to-back via the actual `POST /api/analyses` endpoint. `consolidatedDuplicate` stayed `0` on both runs despite confirmed prior-run audit rows for the same 40 consumers sitting in SQLite in between (`routedToField=40` both times). Calling `case_dedup_guard()` standalone immediately afterward on one of those consumers correctly returned `True` — but that check was contaminated by the just-written row from the run itself, so it doesn't actually prove the loop can see *prior* runs' rows. `recidivist` came back `40` on the same runs, which looks like it works, but wasn't independently verified beyond that — don't trust it further than "looked right once" until someone traces it the same way as dedup.
...
| ~~51.8M real hourly AMI readings ignored~~ | **Fixed** (2026-09-05). Generated `interval_readings.parquet` locally via `python generate_intervals.py` (307.5 MB, ~51.84M rows, 2,000 AMI consumers — matches the audit exactly; stays gitignored, regenerate locally when needed). `investigation_service.get_hourly_readings()` now serves real per-hour data for AMI consumers: `actual` = the consumer's real average hourly profile for the analysis month; `expected` = their real profile from their earliest 3 metered months (always pre-theft-onset per the generator), rescaled to the same daily total. Non-AMI consumers (no real meter) still correctly fall back to the synthesized `_diurnal_shape()` curve — that fallback is legitimate for them. New `Settings.interval_readings_path`/`interval_readings_present()` in `app/config.py` (`ISTIKSHAF_INTERVAL_READINGS` env override); dataset opened once via `functools.lru_cache` and per-consumer reads cached (`maxsize=512`) — first hit ~180ms, cached hits ~20ms. Verified via `scripts/smoke.py` and a direct `/api/investigations/{id}/hourly` round-trip; confirmed the fallback still fires correctly for non-AMI consumers. |
| ~~Pipeline Comparison page hardcoded "68.4%" text~~ | **Fixed** (2026-09-06). Replaced hardcoded "68.4% of connections (6,840 / 10,000)" in `frontend/src/views/PipelineComparisonPage.tsx` with dynamic calculation from backend `comparison.coverageStats` (`smartMeterCovered` / `totalConnections` = 20.0%, 2,000 connections). Also synchronized `AnalystOverviewPage.tsx`'s coverage metric card and tooltip to dynamic `overview.smartMeterCoveragePercentage`, and updated `AnalysisDrawer.tsx` and `AdminDataSourcesPage.tsx` to reflect the true 2,000 active smart-meter population. Verified with `npm run build`. |
| ~~3D Grid Topology is decorative~~ | **Fixed** (2026-09-06). Completely replaced `frontend/src/components/3d/GridMeshCanvas.tsx`'s 180 random sine-wave particles with a real hierarchical 3D distribution network: central 132 kV Substation node (with pulsing glow ring), 6 radial 11 kV Feeder branches, 18 PMT distribution nodes (with high-loss alert indicators styled in alert red), ~108 Consumer clusters, interconnected power line segments, animated energy flow particles along transmission/distribution lines, 3D ground plane grid, and interactive legend overlay with mouse parallax. Verified with `npm run build`. |
